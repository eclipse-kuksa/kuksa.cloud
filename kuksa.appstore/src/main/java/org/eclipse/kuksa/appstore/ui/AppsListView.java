...
public class AppsListView extends CustomComponent implements View {
	...
	public void init() {
		...
		int limit = 9;
		int total;
		Page<App> appsList = findByText(serachText.getValue(),currentpage - 1, limit);
	...
	public Component createPaginationComponent(int total, int currentpage, int limit) {
		...
		pagination.addPageChangeListener(new PaginationChangeListener() {
			...
			public void changed(PaginationResource event) {
				Page<App> appsList = findByText(serachText.getValue(),event.pageIndex(), event.limit());
				CssLayout appslayoutnew = new CssLayout();
				appslayoutnew = crateAppLayout(appsList);
				mainlayout.removeAllComponents();
				mainlayout.addComponent(navHeaderLayout);
	...
	public Page<App> findByText(String text,int page, int size) {
		...
		Page<App> users = appManagerService.findByNameStartsWithIgnoreCase(text,pageable);
		return users;
	}
